...
| KLID 버스 커버리지 부족 (화성·성남·서울 실측 0건, #10) | 버스는 TAGO 버스도착정보(15098530)로 교체 완료(ADR 0001, #13). TAGO 실 키로 화성/동탄 권역 재확인 필요 — 비어 있으면 경기 GBIS 병행 검토. 신호는 교차로 마스터(`crsrd_map_info`)만 서울 K0/2779건 커버 확인됨(#10). **실시간 잔여시간(`tl_drct_info`)은 서울 포함 세 곳 전부 0건이고 전국에서 울산만 제공된다(#30)** — 주기 모델 fallback이 이미 있어 기능은 유지되지만, 실시간 신호 경로는 현재 도달하지 않는다 |
