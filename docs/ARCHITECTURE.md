...
- 그중 `tl_drct_info`가 **`K3`(NODATA)** 로 답한 지자체는 `wio.klid.no-data-ttl`(기본 6시간) 동안
  폴링에서 건너뛴다 (#31). 표시는 메모리에만 두므로 재시작하면 비고, TTL이 지나면 다음 사이클이
  실제로 호출해 커버리지가 생겼는지 확인한다 — 커버리지가 늘어도 설정을 고칠 필요가 없다.
  **`K0`인데 항목이 0건인 응답은 표시하지 않는다.** 그건 "지금 이 순간 보고가 없다"는 일시적
  상태라서, 같이 묶으면 실제로 커버되는 지자체의 관측을 TTL 동안 조용히 잃는다. 이 구분을 위해
  KLID 클라이언트는 `K3`를 빈 목록으로 뭉개지 않고 `noData`로 올린다. 건너뛴 지자체는
  `PollingCycleResult.signalSkippedStdgCds`로 드러난다
- 건너뛰기는 **폴링 경로(`tl_drct_info`)에만** 적용한다. 마스터 동기화(`crsrd_map_info`)는
  사람이 필요할 때 한 번 돌리는 것이라 캐시할 이유가 없다
